...
class BasicAssistant:
    ...
    def __init__(self, intents_data: Union[str, os.PathLike, dict] = None, method_mappings: dict = {}, hidden_layers: list = None, model_name: str = "basic_model") -> None:
    ...
    def _prepare_intents_data(self, ignore_letters: tuple = ("!", "?", ",", ".")):
        ...
        X = list(self.training_data[:,0])
        y = list(self.training_data[:,1])
    ...
    def fit_model(self, optimizer: Optimizer = None, epochs: int = 200):
        ...
        if self.hidden_layers is None:
            ...
            self.model.add(InputLayer(input_shape=(len(X[0]),)))
        ...
        self.history = self.model.fit(np.array(X), np.array(y), epochs=epochs, batch_size=5, verbose=1)
